...
// Grade sparkline Â· Class reminders Â· Emoji reactions
// Mall new-arrival toasts Â· Attendance streak
// ———————————————————————————————————————————————————————————————
'use strict';
...
// Adds a "ð Remind me" button to upcoming class cards.
...
window.scheduleClassReminder = function(classId, classTitle, scheduledAt) {
  ...
  Notification.requestPermission().then(function(perm) {
    ...
    setTimeout(function() {
      new Notification('ð GERAMA Class Starting Soon!', {
...
// Adds a quick-react bar (ð—¤ï¸ðð®ð¢) that floats when hovering a message bubble.
// Reactions are stored per-message in localStorage (client-side only — no DB change needed).
var REACT_EMOJIS = ['ð','—¤ï¸','ð','ð®','ð¢','ð¥'];
...
(function patchBuildMsg(){
  ...
  window.buildMsgHtml = function(m, mine) {
    ...
    var reactBtn = '<button class="msg-react-btn" onclick="event.stopPropagation();window.toggleReactBar(\'' + m.id + '\',this)" style="background:none;border:none;color:#9ca3af;font-size:0.82rem;cursor:pointer;padding:0.2rem 0.3rem;opacity:0.7;transition:opacity 0.2s;" title="React">ð</button>';
...
window.showMallToast = function(productName, sellerName) {
  ...
  toast.innerHTML = '<i class="fas fa-store" style="color:#FFC107;"></i> New arrival: <strong>' + (productName||'Product') + '</strong> by ' + (sellerName||'Seller') + ' ð';
...
// then injects a "ð¥ X-week streak" badge on the dashboard attendance stat box.
    ...
    try {
      ...
      if (attBox && streak >= 2) {
        ...
        if (parent && !parent.querySelector('.att-streak-badge')) {
          ...
          badge.textContent = 'ð¥ ' + streak + '-week streak';
...
// e.g. "Averaging 72% Â· 3 A grades Â· You're doing great! ðª"
window.showGradeSummaryLine = function(grades) {
  ...
  var msg = avg >= 70 ? "You're doing great! ðª" : avg >= 50 ? "Keep pushing — you're almost there! ð" : "Review your weak areas — you can do this! ð±";
  ...
  line.innerHTML = '<i class="fas fa-chart-line"></i> Averaging <strong>' + avg + '%</strong> Â· ' + aCount + ' A+ Â· ' + msg;
