...
(function () {
    ...
    OneSignalDeferred.push(async function (OneSignal) {
        try {
            await OneSignal.init({
                ...
                // Disable the built-in bell — it uses an iframe that iOS PWA blocks.
    ...
    // ── 2. Native bell button (works on iOS PWA — no iframe) ───────
    function _injectNativeBell(OneSignal) {
        ...
        bell.title = 'GERAMA Notifications';
        bell.style.cssText = [
            ...
            try {
                ...
                if (optedIn) {
                    ...
                    if (confirm('You are subscribed to GERAMA notifications.\n\nTap OK to unsubscribe.')) {
                        ...
                        setTimeout(function(){ bell.innerHTML='🔔'; bell.style.background='linear-gradient(135deg,#1B5E20,#2E7D32)'; }, 3000);
                    }
                } else {
                    ...
                    // Show a brief "subscribed" label
                    var tip = document.createElement('div');
                    tip.textContent = '✓ Alerts ON';
                    tip.style.cssText = 'position:fixed;bottom:134px;right:10px;z-index:8001;background:#059669;color:white;padding:0.3rem 0.7rem;border-radius:20px;font-size:0.75rem;font-weight:700;font-family:Inter,sans-serif;pointer-events:none;';
        ...
        // Update bell appearance based on current subscription state
    ...
    async function _maybeShowBanner(OneSignal) {
        try {
            ...
            // iOS not in PWA: show homescreen guide instead
            if (isIOS && !isPWA) {
    ...
    function _injectBanner() {
        ...
        banner.innerHTML =
            ...
            '<div style="flex:1;min-width:0;">' +
                ...
                '<div style="font-size:0.75rem;opacity:0.8;line-height:1.4;">Get instant alerts even when the app is closed.</div>' +
            '</div>' +
            ...
            '<div style="display:flex;align-items:flex-start;gap:0.9rem;">' +
                ...
                '<div style="flex:1;min-width:0;">' +
                    ...
                    '<div style="font-size:0.78rem;opacity:0.9;line-height:1.6;">' +
                        '1. Tap <strong>Share ⬆️</strong> at the bottom of Safari<br>' +
                        '2. Tap <strong>"Add to Home Screen"</strong><br>' +
                        '3. Open GERAMA from your home screen<br>' +
                        '4. Tap <strong>"Allow"</strong> when prompted' +
                    '</div>' +
                '</div>' +
                '<button id="geramaIOSDismiss" style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);color:white;width:28px;height:28px;border-radius:50%;cursor:pointer;font-size:0.9rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-family:Inter,sans-serif;">✕</button>' +
            '</div>';
