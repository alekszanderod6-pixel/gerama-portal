ï»¿/* GERAMA Admin Dashboard â extracted JS â v2 */
...
window.scheduleClass = async function(){
  ...
  try{
    ...
    window.logActivity('Scheduled '+(clsType==='inperson'?'in-person':'virtual')+' class: '+course+' â '+topic);
...
window.submitManualAttendance = async function(){
  ...
  if(secret !== _ATTENDANCE_SECRET){ setS('â Wrong secret code (2026GERAMA). Access denied.','err'); return; }
...
window.removeAttRecord = async function(id, name){
  ...
  if(!secret) return;
  if(secret !== _ATTENDANCE_SECRET){ alert('â Wrong secret code. Access denied.'); return; }
  var sb = window.geramaSupabase; if(!sb) return;
...
window.deleteEntireSession = async function(sessionId, sessionTitle){
  ...
  if(!secret) return;
  if(secret !== _ATTENDANCE_SECRET){ alert('â Wrong code. Access denied.'); return; }
